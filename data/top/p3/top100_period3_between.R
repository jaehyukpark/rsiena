library(igraph)
m2011 <- read.table("market_100_2011.dat",header=F)
m2012 <- read.table("market_100_2012.dat",header=F)
m2013 <- read.table("market_100_2013.dat",header=F)
m2014 <- read.table("market_100_2014.dat",header=F)
m2015 <- read.table("market_100_2015.dat",header=F)

p2011 <- read.table("pro_100_2011.dat",header=F)
p2012 <- read.table("pro_100_2012.dat",header=F)
p2013 <- read.table("pro_100_2013.dat",header=F)
p2014 <- read.table("pro_100_2014.dat",header=F)
p2015 <- read.table("pro_100_2015.dat",header=F)

age3 <- as.matrix(read.table("age_top100_period3.dat"))
ranking3 <- as.matrix(read.table("ranking_top100_period3.dat"))
soloshow3 <- as.matrix(read.table("soloshow_top100_period3.dat"))
award3 <- as.matrix(read.table("award_top100_period3.dat"))
meanprice3 <- as.matrix(read.table("meanprice_top100_period3.dat"))
festbiennal3 <- as.matrix(read.table("festbiennal_top100_period3.dat"))
publicinst3 <- as.matrix(read.table("publicinst_top100_period3.dat"))
countyear3 <- as.matrix(read.table("countyear_top100_period3.dat"))
groupshow3 <- as.matrix(read.table("groupshow_top100_period3.dat"))
...
marketnet3 <-array(c(m2011_adj, m2012_adj, m2013_adj, m2014_adj, m2015_adj), dim=c(98,98,5))
pronet3 <-array(c(p2011_adj, p2012_adj, p2013_adj, p2014_adj, p2015_adj), dim=c(98, 98,5))
...
multiAlg3 <- sienaAlgorithmCreate(projname = 'two_art_networks3')
result_period3top100_between <- siena07(multiAlg3, data = multidata_trial3, nbrNodes=20, effects = multieff3)
xtable(result_period3top100_between, file="result_period3top100_between.htm", type="html")
